          ...
          <div>
            ...
            <div class="flex items-center gap-2">
              <UTooltip :text="$t('theme.mode.system')">
                <UButton :color="isModeActive('system') ? 'primary' : 'neutral'" variant="soft" size="sm" square :class="isModeActive('system')
                    ? 'ring-1 ring-primary-400/40 bg-primary-500/10 dark:bg-primary-400/10'
                    : ''
              ...
              <UTooltip :text="$t('theme.mode.light')">
                <UButton :color="isModeActive('light') ? 'primary' : 'neutral'" variant="soft" size="sm" square :class="isModeActive('light')
                    ? 'ring-1 ring-primary-400/40 bg-primary-500/10 dark:bg-primary-400/10'
                    : ''
                  " icon="i-twemoji-sun" :aria-label="$t('theme.mode.light')" @click="setMode('light')" />
              </UTooltip>

              <UTooltip :text="$t('theme.mode.dark')">
                <UButton :color="isModeActive('dark') ? 'primary' : 'neutral'" variant="soft" size="sm" square :class="isModeActive('dark')
                    ? 'ring-1 ring-primary-400/40 bg-primary-500/10 dark:bg-primary-400/10'
                    : ''
                  " icon="i-twemoji-crescent-moon" :aria-label="$t('theme.mode.dark')" @click="setMode('dark')" />
              </UTooltip>
            </div>
          </div>
          ...
          <div>
            ...
            <div class="grid grid-cols-3 gap-2">
              <UButton v-for="p in primaryItems" :key="p.key" :color="isPrimaryActive(p.key) ? 'primary' : 'neutral'"
                variant="soft" size="sm" :class="[
                  'justify-start',
                  isPrimaryActive(p.key)
                    ? 'ring-1 ring-primary-400/40 bg-primary-500/10 dark:bg-primary-400/10'
                    : '',
                ]" :aria-label="`Primary: ${p.label}`" @click="setPrimary(p.key)">
...
const setPrimary = (p: PrimaryColor) => {
